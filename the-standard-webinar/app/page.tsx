...
export default function Home() {
  ...
  function submitForm(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus("sending");
    // TODO: 등록 API 연동 위치 — 실제 엔드포인트와 성공/실패 처리로 교체하세요.
  ...
  return (
    <>
      <header className="site-header">
        <a className="brand" href="#top" aria-label="페이지 상단으로 이동">[THE STANDARD]</a>
        <nav className="desktop-nav" aria-label="주요 메뉴">
          {navItems.map(([label, href]) => <a key={href} href={href}>{label}</a>)}
        </nav>
        <a className="header-cta" href="#apply">무료 등록하기 <Arrow /></a>
        <button className="menu-button" aria-label="메뉴 열기" aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}>
          <span /><span />
        </button>
      </header>
      <div className={`mobile-menu ${menuOpen ? "is-open" : ""}`} aria-hidden={!menuOpen}>
        <div className="mobile-menu-head"><span>[THE STANDARD]</span><button onClick={() => setMenuOpen(false)} aria-label="메뉴 닫기">×</button></div>
        <nav>{navItems.map(([label, href], index) => <a key={href} href={href} onClick={navigate}><small>0{index + 1}</small>{label}<Arrow /></a>)}</nav>
      </div>
      ...
      <main id="top">
        <section className="hero" aria-labelledby="hero-title">
          ...
          <div className="hero-inner">
            ...
            <h1 id="hero-title"><span className="reveal delay-1">[The Standard]</span><strong className="reveal delay-2">AI 에이전트 시대,</strong><strong className="reveal delay-3">변하지 않는 마케팅의 표준</strong></h1>
            <div className="hero-bottom reveal delay-4">
              ...
              <a className="button button-primary" href="#apply">무료 등록하기 <Arrow /></a>
            </div>
          </div>
          <div className="hero-summary reveal delay-4"><span>THE STANDARD / 01</span><p>변화의 속도보다<br />본질에 집중하는<br />마케팅의 기준.</p><i>SCROLL TO EXPLORE ↓</i></div>
        </section>
        ...
        <section id="about" className="section section-about">
          ...
          <div className="about-card observe">
            ...
            <div className="contrast-grid">
              <article className="change-card"><span>01 / CHANGING</span><div className="liquid" aria-hidden="true" /><h3>변화하는 것</h3><p>AI 기술 · 도구 · 실행의 속도</p></article>
              <article className="standard-card"><span>02 / CONSTANT</span><div className="rule-grid" aria-hidden="true"><i /><i /><i /><i /></div><h3>변하지 않는 것</h3><p>고객 이해 · 전략 · 성과 검증</p></article>
            </div>
          </div>
        </section>
        ...
        <section id="program" className="section section-program">
          ...
          <div className="timetable observe">
            <div className="table-heading"><span>NO.</span><span>TIME</span><span>PROGRAM</span><span>DETAIL</span></div>
        ...
        <section id="apply" className="section section-apply">
          <div className="apply-card observe">
            <div className="apply-intro"><p className="section-kicker">SAVE YOUR SEAT</p><h2>무료로<br />웨비나에<br /><em>등록하세요.</em></h2><p>등록 정보를 남겨주시면 웨비나 참여 방법을 이메일로 안내해 드립니다.</p><dl><div><dt>DATE</dt><dd>2026년 9월 17일 (목)</dd></div><div><dt>FORMAT</dt><dd>온라인 웨비나</dd></div><div><dt>FEE</dt><dd>무료</dd></div></dl></div>
            <div className="form-area">
              ...
              </div><label className={`agreement ${errors.agreement ? "has-error" : ""}`}><input type="checkbox" checked={fields.agreement} onChange={(event) => updateField("agreement", event.target.checked)} /><span>개인정보 수집 및 이용에 동의합니다. <b>(필수)</b></span></label>{errors.agreement && <small className="agree-error" role="alert">{errors.agreement}</small>}<button className="button button-primary submit-button" type="submit" disabled={status === "sending"}>{status === "sending" ? "등록 중입니다..." : <>무료 등록하기 <Arrow /></>}</button><p className="form-note">본 페이지는 데모입니다. 제출 정보는 저장되지 않습니다.</p></form>}
            </div>
          </div>
        </section>
      </main>
      <footer><div><span className="footer-wordmark">[THE STANDARD]</span><p>AI 에이전트 시대, 변하지 않는 마케팅의 표준<br />2026. 09. 17 (THU) · ONLINE WEBINAR</p></div><div className="footer-right"><span className="host-placeholder">HOST LOGO</span><a href="#top">BACK TO TOP ↑</a><small>© 2026 THE STANDARD. ALL RIGHTS RESERVED.</small></div></footer>
    </>
  );
}
